...
void usage() {
    ...
    printf("\t-F <1-254>\tLowest RTU id.\n");
    printf("\t-T <2-255>\tHighest RTU id.\n");
...
int main(int argc, char *argv[]) {
    ...
    while ((ch = getopt( argc, argv, "b:d:hl:?p:r:s:t:vF:T:")) != -1 ) {
        switch (ch) {
            case 'F':   // lowest RTU id;
                modbus_start=atoi( optarg );
                
                if (modbus_start < 1) {
                    fprintf(stderr,"Warning: lowest RTU being set to 1\n");
                    modbus_start=1;
                }
                break;
            case 'T':   // highest RTU id
                modbus_end=atoi(optarg);
                
                if (modbus_end > 255) {
                    fprintf(stderr,"Warning: highest RTU being set to 255\n");
                    modbus_end=255;
                }
                break;
            case 'b':   // baud rate
    ...
    if (modbus_start >= modbus_end) {
        fprintf(stderr,"FATAL ERROR: Last RTU id MUST be greater than first RTU id.\n");
        exit(-1);
    }
    ...
    for (rtu=modbus_start; rtu <= modbus_end; rtu++) {
        /*
         * Set modbus RTU id.
         */
