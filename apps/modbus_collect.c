...
int main() {
     ...
     * constraint is enforced by the database, so the following SQL statement,
     * select the rows by descinding index, i.e. the newest first,
            ...
            if( rtuArray[i] == -1 ) {
                ...
                while (retries > 0) {
                    ...
                    if (rc > 0) {
                        ...
                        printf("=============================\n");
                        */
                        sprintf(sql,"insert into data (RTU,phaseA,phaseB,phaseC) values( %d,%d,%d,%d);", rtu,tab_rp_registers[L1_POWER - modbus_base_address],tab_rp_registers[L2_POWER - modbus_base_address],tab_rp_registers[L3_POWER - modbus_base_address]);
                        printf("%s\n", sql);
                        ...
                        do {
                            ...
                            if (rc == SQLITE_BUSY) {
                                printf("++++ Locked in insert, waiting.\n");
                                usleep ( 500 MS );
                            }
                        } while (rc == SQLITE_BUSY );
                        /*
